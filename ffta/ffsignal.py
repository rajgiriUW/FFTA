"""ffsignal.py: Contains FFSignal class."""
...
class FFSignal:
    ...
    def __init__(self,
        ...
        if not FFSignal._deprecation_warned:
            warnings.warn('Pixel is deprecated and will be removed in the next major release. '
                          'Use FFSignal instead.',
                          category=DeprecationWarning, stacklevel=2)
            FFSignal._deprecation_warned = True
    ...
    def update_parm(self, **kwargs):
        """
        Update the parameters. See :class:`FFSignal` for valid parameter names.
        e.g. ``p.update_parm(method='wavelet')``

        Parameters
        ----------
        **kwargs
            Keyword arguments matching existing FFSignal attributes.
        """
    ...
    def fir_filter(self):
        ...
        if cache_key not in FFSignal._fir_tap_cache:
            try:
                FFSignal._fir_tap_cache[cache_key] = sps.firwin(int(self.n_taps), band,
        ...
        self.signal = sps.fftconvolve(self.signal, FFSignal._fir_tap_cache[cache_key], mode='same')
